  ...
  public async validate(blocks: RawSubjectBlock[]): Promise<COImportPayload> {
    ...
    for (const [subjCode, block] of uniqueSubjectsMap.entries()) {
      ...
      if (!deptConfig) {
        ...
        const dbDept = dbDepts.find(d => d.departmentName.toUpperCase() === deptConfig.departmentCode || d.departmentName.toUpperCase() === deptConfig.departmentName.toUpperCase());
        if (dbDept) {
      ...
      if (dbReg) {
         ...
         validated.issues.push(`Error: Regulation '${block.parsedSubjectCode.regulationCode}' not found in DB.`);
         validated.action = 'CONTEXT_CONFLICT';
      }
      ...
      const dbSem = dbSemesters.find(s => s.semester === mappedSemName);
      if (dbSem) {
         validated.semesterId = dbSem.id;
      } else {
         validated.issues.push(`Error: Semester '${mappedSemName}' (from absolute semester ${absoluteSem}) not found in DB.`);
         validated.action = 'CONTEXT_CONFLICT';
      }
      ...
      const existingSubject = await prisma.subject.findUnique({ 
        ...
        include: { courseOutcomes: true, department: true, regulation: true, semester: true }
      });

      if (existingSubject && validated.action !== 'CONTEXT_CONFLICT') {
         // Identity checks
         ...
         } else if (existingSubject.departmentId !== validated.departmentId || 
                    existingSubject.regulationId !== validated.regulationId || 
                    existingSubject.semesterId !== validated.semesterId) {
            validated.action = 'CONTEXT_CONFLICT';
            validated.issues.push('Context Conflict: Academic context differs from existing DB record.');
         } else {
      ...
      // Process COs
      const uniqueCosMap = new Map<string, string>();
      for (const co of block.courseOutcomes) {
         if (!uniqueCosMap.has(co.coCode)) {
            uniqueCosMap.set(co.coCode, co.description);
         }
      }

      for (const [coCode, description] of uniqueCosMap.entries()) {
         const vCo: ValidatedCO = { coCode, description, action: 'CREATE', issues: [] };
         if (existingSubject) {
            const exCo = existingSubject.courseOutcomes.find(c => c.coCode === coCode);
            if (exCo) {
