...
export default function InternalReportPage() {
  ...
  const handleGenerate = async () => {
    if (!file) {
      setError('Please upload the Internal Marks Excel template.');
      return;
    }
    if (!selectedSubject) {
    ...
    try {
      const { blob, historyId } = await facultyService.generateInternalReport(file, selectedSubject);
      ...
      a.href = url;
      a.download = `Generated_Internal_OBE_Report_${selectedSubject}.xlsx`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      a.remove();

      setSuccessMsg(`Internal OBE Report for ${selectedSubject} generated and downloaded successfully!`);
    } catch (err: any) {
      setError(err.message || 'Error occurred while generating Internal report.');
    } finally {
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '28px' }}>
      <div>
        <h1 style={{ fontSize: '1.8rem', color: '#f8fafc', marginBottom: '8px' }}>
          Internal Marks OBE Generator
        </h1>
        <p style={{ color: '#94a3b8', fontSize: '0.95rem' }}>
          Upload faculty mid-term marks Excel workbook to calculate Course Outcome direct attainment and paste into the institutional template.
        </p>
      </div>
      ...
      <Card glow>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div>
            ...
            >
              Upload Internal Marks Excel
            </label>
            <Dropzone
              selectedFile={file}
              onFileSelect={(selected) => setFile(selected)}
              onClear={() => setFile(null)}
              title="Upload Internal Marks Workbook (.xlsx)"
              subtitle="Must follow the standard MIC Internal Marks format"
            />
          </div>
          ...
          <div style={{ display: 'flex', gap: '12px', marginTop: '8px', alignItems: 'center' }}>
            <Button
              ...
              style={{ flex: 1 }}
            >
              🚀 Generate & Download Internal Report
            </Button>
