...
export default function ExternalReportPage() {
  ...
  const handleGenerate = async () => {
    if (!file) {
      setError('Please upload the External Marks Excel template.');
      return;
    }
    if (!selectedSubject) {
    ...
    try {
      const { blob, historyId } = await facultyService.generateExternalReport(file, selectedSubject);
      ...
      a.href = url;
      a.download = `Generated_External_OBE_Report_${selectedSubject}.xlsx`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      a.remove();

      setSuccessMsg(`External OBE Report for ${selectedSubject} generated and downloaded successfully!`);
    } catch (err: any) {
      setError(err.message || 'Error occurred while generating External report.');
    } finally {
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '28px' }}>
      <div>
        <h1 style={{ fontSize: '1.8rem', color: '#f8fafc', marginBottom: '8px' }}>
          External Marks OBE Generator
        </h1>
        <p style={{ color: '#94a3b8', fontSize: '0.95rem' }}>
          Upload external end-semester theory marks Excel workbook (Part A & Part B) to compute direct external attainment.
        </p>
      </div>
      ...
      <Card glow>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div>
            ...
            >
              Upload External Marks Excel
            </label>
            <Dropzone
              selectedFile={file}
              onFileSelect={(selected) => setFile(selected)}
              onClear={() => setFile(null)}
              title="Upload External Marks Workbook (.xlsx)"
              subtitle="Must follow the standard External Question-wise Template"
            />
          </div>
          ...
          <div style={{ display: 'flex', gap: '12px', marginTop: '8px', alignItems: 'center' }}>
            <Button
              ...
              style={{ flex: 1 }}
            >
              🚀 Generate & Download External Report
            </Button>
